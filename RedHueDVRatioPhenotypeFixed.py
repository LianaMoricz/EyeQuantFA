...
def calcImage(imagePath, rangeWidth=0, scalerAdj=50):
    ...
    if np.any(valid):
        ...
        # Using the midpoint of the vertical extent
        ...
        # The slope should be based on the oval's dimensions and position
        yIndices, xIndices = np.indices((height, width))
        
        # Calculate slope based on oval's actual center
        # We want the slope line to pass through the center of the oval
        slopeFactor = 0.4
        verticalOffsetPercent = 0.10
        
        # Calculate the slope relative to the oval's center
        # Shift coordinates so the oval's center is at origin
        ...
        # But adjusted for the oval's aspect ratio
